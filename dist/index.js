...
class spins {
    ...
    start(...text) {
        ...
        //@ts-ignore
        var spinner = cli_spinners_1.default[opts.spinner] || opts.spinner;
        this.text = text.join(" ");
        this.interval = setInterval(() => {
            opts.stream.clearScreenDown();
            //if has a color
            if (opts.color)
                opts.stream.write(`\r${chalk_1.default[opts.color](spinner.frames[this.index++])} ${text.join(" ")}`);
            //else write the spinner without color
            else
                opts.stream.write(`\r${spinner.frames[this.index++]} ${text.join(" ")}`);
            //if index is greater than the length of the frames array, reset it
            if (this.index === spinner.frames.length)
                this.index = 0;
        }, spinner.interval);
        return this;
    }
